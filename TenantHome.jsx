...
function TenantHome() {
  ...
  async function handleVacateRequest(tenancyId) {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        throw new Error(json.message || 'Failed to submit vacate request');
      }
    ...
    <div className="min-h-screen bg-slate-50 flex">
      <main className="flex-1 max-w-3xl mx-auto p-4 md:p-8 space-y-6">
        <header className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-slate-800">My Home</h1>
            <p className="text-sm text-slate-500">
              View your rent status, wallet balance, and bills.
            </p>
          </div>
        </header>
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-slate-500 mt-2">
                    Status: <StatusPill status={activeTenancy.status} /> • Rent:{' '}
                ...
                {activeTenancy.wallet ? (
                  <div className="space-y-2 text-sm">
                    <div>
                      ...
                      <div className="font-mono text-slate-900">123456</div>
                    </div>
                    <div>
                ...
                {activeTenancy.latestBill ? (
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 text-sm">
                    ...
                    <div className="flex flex-col items-start md:items-end">
                      ...
                      <div className="text-xs text-slate-500 mt-1">
                        Paid: {formatCurrency(activeTenancy.latestBill.amountPaid)} • Balance:{' '}
                        {formatCurrency(activeTenancy.latestBill.balance)}
                      </div>
